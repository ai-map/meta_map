 ...
 * 事件类型枚举
 */
export type EventType = 'add' | 'remove' | 'update' | 'cluster';
...
export abstract class ClusterManager<T extends Point = Point> {
  ...
  protected metersToLongitude(meters: number, latitude: number): number {
    ...
    return meters / (EARTH_RADIUS * 1000 * Math.cos(latRad)) * (180 / Math.PI);
  }
} 