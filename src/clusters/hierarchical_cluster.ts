 ...
 */
export class HierarchicalClusterManager<T extends Point = Point> extends ClusterManager<T> {
  /**
   * 执行层次聚类算法
   * @param points 需要聚类的点
   * @param options 聚类选项
   ...
   * @returns 聚类结果
   */
  private divideAndCluster(points: T[], options: ClusterOptions, depth: number): Cluster<T>[] {
    ...
    // 将点按分割轴排序
    const sortedPoints = [...points].sort((a, b) => a[splitAxis] - b[splitAxis]);
    ...
    const rightClusters = this.divideAndCluster(rightPoints, options, depth + 1);
   ...
   * @returns 合并后的聚类列表
   */
  private mergeClusters(clusters: Cluster<T>[], maxDistance: number): Cluster<T>[] {
      ...
      while (merged) {
        ...
        for (let j = 0; j < clusters.length; j++) {
          ...
          if (distance <= maxDistance) {
            // 合并点
            const mergedPoints = [...currentCluster.points, ...clusters[j].points];
            // 计算新的中心点
  ...
  private calculateGeoBounds(points: T[]): { 
    ...
    if (points.length === 0) {
      return { minLon: 0, minLat: 0, maxLon: 0, maxLat: 0, width: 0, height: 0 };
    }
