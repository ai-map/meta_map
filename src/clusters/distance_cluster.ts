 ...
 */
export class DistanceClusterManager<T extends Point = Point> extends ClusterManager<T> {
  /**
   * 执行基于距离的聚类算法
   * @param points 需要聚类的点
   * @param options 聚类选项
  ...
  protected performClustering(points: T[], options: ClusterOptions): Cluster<T>[] {
    ...
    for (const point of points) {
      ...
      // 找到当前点半径范围内的所有点
      const nearbyPoints = this.findNearbyPoints(point, points, radius, processedPoints);
